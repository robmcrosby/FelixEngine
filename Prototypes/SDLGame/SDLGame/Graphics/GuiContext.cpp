...
GuiContext::GuiContext(): mDevice(VK_NULL_HANDLE), mDescriptorPool(VK_NULL_HANDLE), mCheckboxSelection(false), mFloatValue(0.5f), mButtonCount(0) {}
...
bool GuiContext::init(GpuContext& gpuContext) {
  ...
  //io.Fonts->AddFontFromFileTTF("Stencilia-A.ttf");
  //io.Fonts->AddFontFromFileTTF("AnchorJack.ttf");
  //io.Fonts->AddFontFromFileTTF("Times New Normal Regular.ttf");
  //io.Fonts->AddFontFromFileTTF("SuperCrawler.ttf");
...
ImDrawData* GuiContext::drawUI(EcsScene& scene) const {
  ImGui_ImplVulkan_NewFrame();
  ImGui_ImplSDL3_NewFrame();
  ImGui::NewFrame();
  
  auto panels = scene.registry().view<GuiPanelComp, GuiBoundsComp, TagComp>();
  auto widgets = scene.registry().view<GuiWidgetComp, ParentComp>();
...
void GuiContext::draw(VkCommandBuffer cmd, EcsScene& scene) const {
...
void GuiContext::setNextPanel(const GuiPanelComp &panel) const {
...
void GuiContext::beginPanel(GuiPanelComp& panel, const string& name) const {
  setNextPanel(panel);
...
void GuiContext::endPanel(GuiBoundsComp& bounds) const {
...
void GuiContext::addWidget(EcsEntity widget, GuiWidgetComp& widgetComp) const {
  switch (widgetComp.type) {
    case GuiLabel:
      ImGui::Text("%s", widgetComp.text.c_str());
      break;
    case GuiButton:
      if (ImGui::Button(widgetComp.text.c_str()))
        widget.enqueue(ButtonPressEvent);
      break;
    default:
      break;
  }
