...
void VulkanSwapChain::setToWindow(SDL_Window *window) {
  ...
  if (!SDL_Vulkan_CreateSurface(mSdlWindow, instance, NULL, &mVkSurface))
...
VkSurfaceFormatKHR VulkanSwapChain::pickSurfaceFormat() const {
  ...
  for (const auto& format : surfaceFormats) {
    ...
    if (format.format == VK_FORMAT_R16G16B16A16_SFLOAT && format.colorSpace == VK_COLOR_SPACE_DISPLAY_P3_LINEAR_EXT)
      return format;
  }
  for (const auto& format : surfaceFormats) {
...
void VulkanSwapChain::presentFrame(uint32_t frame, VkSemaphore semaphore, VkQueue queue) {
...
void VulkanSwapChain::presentFrame(VulkanFrameSyncPtr frameSync, VulkanQueuePtr queue) {
  presentFrame(mCurrentFrame, frameSync->finishedSemaphore(), queue->getVkQueue());
