...
vertex VertexOutput skybox_vertex(VertexInput vert [[stage_in]],
  ...
  VertexOutput output;
  output.position = MVP.projection * MVP.view * float4(vert.position, 1.0);
  
  //float3 normal = normalize(rotate_quat(MVP.rotation, vert.normal));
  float3 view = normalize(vert.position - MVP.camera.xyz);
...
fragment half4 basic_fragment(FragmentInput      input       [[stage_in]],
  ...
  //return half4(cubeMap.sample(cubeSampler, input.texCoords, level(10)));
  return half4(cubeMap.sample(cubeSampler, input.texCoords));
  //return half4(cubeMap.sample(cubeSampler, input.texCoords) - float4(1.0, 1.0, 1.0, 0.0)) * 0.5;
  //float a = cubeMap.sample(cubeSampler, input.texCoords).w;
